...
<div class="relative pl-6 border-l-2 border-white/10 hover:border-primary/60 transition-colors group">
  <!-- Timeline dot -->
  <div class="absolute -left-[7px] top-1 w-3 h-3 rounded-full bg-dark border-2 border-white/20 group-hover:border-primary group-hover:bg-primary/20 transition-colors"></div>

  <div class="p-6 rounded-2xl bg-white/[0.03] border border-white/5 hover:border-primary/30 transition-all duration-300 hover:shadow-[0_0_30px_rgba(59,130,246,0.06)]">
    <!-- Header row -->
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-1">
      <div class="space-y-1">
        ...
        <p class="text-lg font-medium text-gray-200">{subtitle}</p>
        {#if field}
          <p class="text-sm text-gray-400">{field}</p>
        {/if}
      </div>

      <div class="flex flex-col items-start sm:items-end gap-1 shrink-0">
        <div class="px-3 py-1 rounded-full bg-white/5 text-sm text-gray-400 whitespace-nowrap font-mono">
    ...
    <!-- Summary -->
    {#if summary}
      <p class="text-gray-400 leading-relaxed mt-3 text-[0.95rem]">{summary}</p>
    {/if}
    ...
    {#if achievements.length > 0}
      <ul class="mt-4 space-y-2">
        {#each achievements as point}
          <li class="flex items-start gap-2 text-gray-300 text-sm leading-relaxed">
    ...
    {#if technologies.length > 0}
      <div class="mt-4 flex flex-wrap gap-1.5">
        {#each technologies as tech}
          <span class="px-2.5 py-0.5 text-xs font-medium rounded-full bg-white/5 text-gray-400 border border-white/5 group-hover:border-primary/15 group-hover:text-gray-300 transition-colors">
            {tech}
          </span>
        {/each}
      </div>
